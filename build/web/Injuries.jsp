...
<body id="home" data-spy="scroll" data-target="#navbar-wd" data-offset="98">
  ...
  <% for(Governorate g:governorates){ 
      List<Sick> injuries=SickApi.get_injuries(g.governorate_id);
      List<Sick> healthies=SickApi.get_healthies(g.governorate_id);
      List<Sick> deads=SickApi.get_deads(g.governorate_id);
  %>
  <tr>
      <td><%= g.governorate_name %></td>
      <td><%= injuries.size()%></td>
      <td><%= deads.size() %></td>
      <td><%= healthies.size()%></td>
  </tr>
  <%}%>
</table>
                    </div>
